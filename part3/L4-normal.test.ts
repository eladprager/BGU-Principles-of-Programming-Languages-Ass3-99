...
import { makeOk, isOk, Result, bind, makeFailure } from "../shared/result";
                ...
                (define test 
                    (lambda (x y) 
                        (if (= x 0)
                            0
                            y)))
                (define zero-div (lambda (n) (/ n 0)))
                (test 0 (zero-div 1)))`), evalNormalProgram)).to.deep.equal(makeOk(0));

    });

    it('test 2 - infinite loop', () => {
        expect(bind(parseL4(`
            (L4 
                (define loop (lambda (x) (loop x)))
                (define g (lambda (x y) y))
                (g (loop 0) 1))`), evalNormalProgram)).to.deep.equal(makeOk(1));
    ...
    it('test 3 - display', () => {
        expect(bind(parseL4(`
            (L4 
                ...
                (test (applic)))`), evalNormalProgram)).to.deep.equal(makeOk(1));
    });
});
